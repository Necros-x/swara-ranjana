...
alter table public.events
  add column if not exists admission_time_enforced boolean not null default false;
...
begin
  ...
  elsif found_ticket.admission_time_enforced
        and v_admission_slot = 'SCHOOL'
        and (v_valid_from is null or v_valid_until is null) then
    result := 'NOT_YET_VALID';
  elsif found_ticket.admission_time_enforced
        and v_valid_from is not null
        and v_now < v_valid_from then
    result := 'NOT_YET_VALID';
  elsif found_ticket.admission_time_enforced
        and v_valid_until is not null
        and v_now > v_valid_until then
    result := 'EXPIRED';
  ...
  insert into public.scan_logs(
    ...
    jsonb_build_object(
      ...
      'time_enforced',found_ticket.admission_time_enforced
    )
  );
  ...
  response := jsonb_build_object(
    ...
    'timeEnforced', found_ticket.admission_time_enforced
  );
