...
begin
  ...
  if p_quantity is null or p_quantity < 1 or p_quantity > 20 then
    return jsonb_build_object(
      ...
      'message', 'Choose a valid ticket quantity.'
    );
  end if;
  ...
  if not v_event.web_ticketing_enabled then
    return jsonb_build_object(
      'ok', false,
      'code', 'WEB_TICKETING_DISABLED',
      'message', 'Online ticketing is not enabled for this show.'
    );
  end if;
  ...
  if not found then
    return jsonb_build_object(
      ...
      'message', 'That ticket category is no longer available.'
    );
  end if;
  ...
  if v_ticket_type.status <> 'AVAILABLE' then
    return jsonb_build_object(
      ...
      'message', 'That ticket category is currently unavailable.'
    );
  end if;

  if v_ticket_type.sale_starts_at is not null
     and v_ticket_type.sale_starts_at > now() then
    return jsonb_build_object(
      'ok', false,
      'code', 'SALE_NOT_STARTED',
      'message', 'Sales for this ticket category have not started yet.'
    );
  end if;

  if v_ticket_type.sale_ends_at is not null
     and v_ticket_type.sale_ends_at < now() then
    return jsonb_build_object(
      'ok', false,
      'code', 'SALE_ENDED',
      'message', 'Sales for this ticket category have ended.'
    );
  end if;
  ...
  if p_quantity > v_ticket_type.max_per_order then
    return jsonb_build_object(
      ...
      'code', 'MAX_PER_ORDER',
      'message', format(
        'A maximum of %s tickets can be reserved in one order.',
        v_ticket_type.max_per_order
      )
    );
  end if;
  ...
  select
    array_agg(q.seat_id order by q.serial_number desc),
    count(*)::integer
  into v_seat_ids, v_seat_count
  from (
    select i.seat_id, i.serial_number
    from public.seat_ticket_inventory i
    join public.event_seats s on s.id = i.seat_id
    where i.event_id = p_event_id
      and i.ticket_type_id = p_ticket_type_id
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'allocation_direction', 'descending'
    )
  )
