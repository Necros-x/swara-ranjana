-- Phase 13D: remove the retired exact-seat/manual-fee workflow while
-- keeping the real auditorium seat inventory and automatic seat assignment.
...
begin
  ...
  -- Expired holds should no longer block the physical seat inventory.
  ...
  -- Random automatic assignment only. Rows are locked while the reservation is
  -- created so concurrent checkouts cannot receive the same physical seat.
...
revoke all on function public.create_checkout_reservation(uuid, uuid, integer, text, text, text, text, uuid) from public;
grant execute on function public.create_checkout_reservation(uuid, uuid, integer, text, text, text, text, uuid) to service_role;

-- Remove the server/API surface for retired exact-seat selection and seat fees.
drop function if exists public.create_checkout_reservation_with_seats(uuid, uuid, integer, text, text, text, text, uuid, text, uuid[]);
drop function if exists public.get_public_seat_map(uuid, uuid);
drop function if exists public.update_seat_pricing_server(uuid, uuid, text, integer, integer);

-- Remove obsolete metadata left by the old random/manual selector.
update public.orders
set metadata = coalesce(metadata, '{}'::jsonb)
  - 'seat_selection_mode'
  - 'seat_selection_fee_lkr'
where coalesce(metadata, '{}'::jsonb) ? 'seat_selection_mode'
   or coalesce(metadata, '{}'::jsonb) ? 'seat_selection_fee_lkr';

-- Keep the real seat inventory, holds and ticket seat labels, but delete every
-- column that existed only for exact-seat pricing/manual selection.
alter table public.order_items
  drop column if exists seat_selection_fee_lkr;

alter table public.seat_blocks
  drop column if exists manual_fee_lkr;
...
alter table public.order_seats
  ...
  drop column if exists surcharge_lkr;
