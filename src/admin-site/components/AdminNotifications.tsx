    ...
    <div ref={rootRef} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-12 z-[80] w-[min(360px,calc(100vw-2rem))] overflow-hidden rounded-xl border border-[#C2CBD2]/60 bg-white shadow-[0_18px_50px_rgba(14,23,33,0.14)]">
          <div className="flex items-center justify-between border-b border-[#C2CBD2]/35 px-4 py-3">
            <div>
            ...
            {items.length > 0 && (
              <span className="rounded-full bg-[#0E1721] px-2 py-1 text-[9px] font-bold text-white">
                {items.length} active
              </span>
            )}
          </div>
            ...
            {items.length === 0 ? (
              ...
              items.map((item) => (
                <Link
                  ...
                  className="flex cursor-pointer gap-3 border-b border-[#C2CBD2]/25 px-4 py-4 transition last:border-0 hover:bg-[#F8FAFB]"
                >
                  <span className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${toneClass(item.tone)}`} />
